...
description: >-
  The permanent income model of Friedman and Hall, covering the savings problem,
  martingale consumption, cointegration, impulse responses, and cross-sectional implications.
keywords:
  - permanent income
  - consumption
  - random walk
...
Regarding the endowment process, we assume it has the [state-space representation](https://python.quantecon.org/linear_models.html)
...
as in the [LQ control lecture](https://python.quantecon.org/lqcontrol.html).  We do this later. But first we use
...
In fact, from the [discussion on forecasting geometric sums](https://python.quantecon.org/linear_models.html#prediction), we see that
...
We can use the following formulas from [linear state space models](https://python.quantecon.org/linear_models.html) to compute population mean $\mu_t = \mathbb{E}    x_t$ and covariance $\Sigma_t := \mathbb{E} [ (x_t - \mu_t) (x_t - \mu_t)']$
...
Recalling again our discussion on [forecasting geometric sums](https://python.quantecon.org/linear_models.html#prediction), we have
...
distribution dynamics in the [lecture on linear systems](https://python.quantecon.org/linear_models.html).
...
Please see the [Kalman filter lecture](https://python.quantecon.org/kalman.html) for more details.
...
[^fn_as]: This would be the case if, for example, the spectral radius of $A$ is strictly less than one.
